...
![](/ai-articles/2026-04-19/og-a2ui-v0-9-generative-ui.png)
...
Google 把 A2UI v0.9 定位成這個問題的答案，也就是一套與框架無關、用來宣告 UI 意圖的標準。它讓本地或遠端代理都能用共同語言和任何客戶端應用程式溝通，確保 agent 可以在任何裝置上，透過既有的元件目錄來生成 UI，而不是逼團隊重做整個前端。
...
- **更簡化的傳輸層：** Google 重新整理了傳輸介面，讓 agent 與 client 的接線方式更順。A2UI 現在可以跑在 MCP、WebSocket、REST、AG-UI、A2A，或任何你想接的 transport 上。
...
Google 強調，前面的範例只是靜態整合示意，真正的 A2UI Agent SDK 已經是朝正式上線等級的複雜場景設計。它預設支援幾個重要能力：
