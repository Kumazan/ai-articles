...
除此之外，幾乎也沒有其他有意義的度量方式。你可以在特定基準測試裡量 tokens per second，也可以算輸入和輸出 token 的比例，但這又不太說得通。至少在雲端運算這種比較得上號的服務裡，你知道自己買的是多少算力、記憶體、儲存與連線。你還是得小心自動化失控或管理不當，Bill Shock 在 AWS 也一樣會咬人，但至少你還能把結果和成本連起來。LLM 服務就難多了，AI agent 更是如此。
...
遷移是最難開口的一個字。就算有一整電話簿那麼多指標證明該換，也一樣難。你可以算每個 instance 的成本、每 TB 的成本，或是維持商業模式所需的成本，也許還不至於完全錯。但這套邏輯放到 AI 密集型的 CI/CD 上會怎麼運作，恐怕還是交給別人先想清楚比較好。
