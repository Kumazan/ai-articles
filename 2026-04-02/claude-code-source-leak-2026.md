...
- 程式碼揭露了 44 個未發布的隱藏功能，包括 KAIROS 自主背景代理、ULTRAPLAN 雲端長時間推理、BUDDY Tamagotchi 數位寵物，以及名為「Capybara」的下一代模型家族
...
- **04:23 UTC** — Chaofan Shou（@Fried_rice，Solayer Labs 實習生）在 Twitter 上發文，直接附上的下載連結。1,600 萬人湧入該推文。
...
| 隱藏功能旗標 | 44 |
| Claude Code ARR | $25 億美元 |
| Anthropic 總 ARR | $190 億美元 |
| 原始推文瀏覽量 | 1,600 萬 |
...
這份洩漏的程式碼是 Claude Code 的 `src/` 目錄——也就是那個「agentic harness」，用於包裝底層的 Claude 模型，赋予它工具使用、檔案管理、bash 命令執行，以及多代理工作流統籌的能力。這不是模型權重（那些沒有暴露），但在許多方面，這是更具策略價值的東西。
...
Anthropic 是自稱「安全優先的 AI 實驗室」。他們在 2025 年 11 月的員工演說中強調了「資訊隔離牆」與「負責任的模型部署」。他們從不發布產品原始碼。如果他們真的想「意外」洩漏大量戰略資產的同時，還能同時轉移焦點到 axios 攻擊上並同時公佈一個 $190 億 ARR 的安全公告——那這將是史上最精密的公關操作之一。
...
對於所有在建构 AI 應用的開發者來說，這份程式碼是一座金礦。不是因為可以抄襲，而是因為可以學習：一家以安全著稱的公司，如何設計一個能長時間自主運作、卻不會迷失在上下文中的系統？三層記憶體機制的設計細節，可能是過去一年來最重要的 AI 工程文獻。
