...
![](/ai-articles/2026-03-29/og-stop-calling-it-memory-obsidian.png)
...
## 技術上正確，但忽略了「夠用」在什麼場景是真正的答案

這篇文章的核心論點——資料庫存知識、markdown 存指令——在技術上幾乎無懈可擊。SQLite 能查詢、能排序、能強制 schema、能處理並行存取，markdown 檔案做不到這些事。955 筆結構化記憶加上 726 個節點的圖資料庫，確實比一堆 .md 檔案強太多。

但有幾件事值得退後一步看。

**第一，「記憶」這個詞的語義光譜比文章承認的更寬。** 作者把 AI 記憶的定義鎖死在「可查詢的結構化知識」，然後論證 markdown 做不到——這當然做不到。但對很多使用者來說，「記憶」更接近「下次 Claude 開啟 session 時，能記得上次聊到哪」。在這個層次，一份 CLAUDE.md 或 MEMORY.md 確實能運作，而且運作得不差。文章用 955 筆記錄的需求去衡量 50 筆記錄的使用場景，結論自然是 markdown 不堪用——但這不代表每個人的使用規模都會到 955 筆。

**第二，文章對 OpenClaw 的批評忽略了一個關鍵脈絡。** 作者指出 OpenClaw 用 markdown 做記憶儲存，然後又加裝 SQLite 索引與向量搜尋，藉此論證「連 OpenClaw 自己都知道 markdown 不夠用」。但這其實恰好說明了一種務實的架構選擇：用 markdown 作為人類可讀可編輯的介面層，用 SQLite/embedding 作為底層查詢引擎。這不是「承認失敗後的補丁」，更像是故意設計的雙層架構——面向人的那層用 markdown，面向機器的那層用資料庫。

**第三，便利貼類比雖然生動，但過度簡化了實際的工程取捨。** 沒有人會拿便利貼管理 955 筆客戶資料，這點毫無爭議。但如果便利貼只有 20 張、而且每張都能被直接讀進 context window，那它比打開一個 SQL client 下查詢確實更快、更直覺、更容易 debug。在 AI 工具鏈裡，「人類可以直接打開檔案看到 AI 知道什麼」這個特性不是廢話，它是可觀察性（observability）的最低成本實現。

歸根結底，這篇文章的最大價值，是打破「markdown 資料夾就是 AI 記憶系統」這個被內容創作者過度放大的迷思。在這個意義上，它非常正確也非常必要。但如果因此得出「所有人都該用 SQLite + 圖資料庫」的結論，那就從一個極端跳到另一個極端。**真正成熟的判斷，是根據你實際的資料量、查詢需求和維護能力，選擇對應層級的基礎設施——而不是一律搬出最重的工具。**
