...
![](/ai-articles/2026-03-29/og-openai-model-spec-approach.png)
...
Model Spec 的核心機制是「指令鏈」：OpenAI > 開發者 > 用戶，高層指令優先。設計邏輯很清楚，但它本質上是一個**由 OpenAI 單方面定義、單方面解釋、單方面執行**的框架。哪些行為是「硬性規則」、哪些是「可覆寫的預設」，由 OpenAI 決定。當模型行為出問題時，是 bug 還是 feature，也由 OpenAI 對照 Spec 來判斷。
...
對開發者和使用者來說，Model Spec 最大的實用價值，可能不是它作為道德文件的意義，而是它作為**溝通工具**的功能：當你覺得模型行為不對勁，你可以指著 Spec 的某一段說「這不符合你們自己寫的規則」。這種可引用性確實比 Anthropic 的 Constitution 更具可操作性。但可引用和可問責之間的距離，仍然需要更多獨立的外部監督來彌補。
