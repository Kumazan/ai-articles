...
- **自訂樣式**：`assets/css/style.scss`（覆蓋 theme 預設，定義設計系統與共用元件）
...
assets/css/style.scss             # 全域樣式與共用元件
_config.yml                       # Jekyll 設定
_includes/head-custom.html        # 自訂 <head>（字型、viewport 覆寫腳本）
```
...
建議填寫欄位：
- `author: 原作者名稱`
- `description: "60-100 字繁中摘要，用於社群分享預覽"`

選填欄位：
- `image: /YYYY-MM-DD/og-article-slug.png`（文章主視覺或 OG image 路徑，預留給自訂模板或社群分享設定使用）

## 文章 Body 結構（必須依序包含，缺一不可）

⚠️ **新文章與後續修改的文章，一律禁止在 body 寫 `# 標題`（H1）**：`post` layout 會從 frontmatter `title` 自動渲染頁面標題，再加 H1 會造成標題重複。舊文章若仍含有 body H1，會視情況逐步調整，無需比照。
...
3. 填寫 frontmatter（4 個必填欄位，建議加上 author/description）+ body（依上述結構）
...
7. **Frontmatter 格式** — 4 個必填欄位齊全（title / date / layout / permalink）；`author`、`description` 建議填寫
...
- ❌ 不要刪除或修改 `_includes/head-custom.html` 中的 viewport / scale 修正（防止 iOS 文字自動縮放）
- ❌ 不要在單篇文章、inline style 或額外樣式中另外為 body 設 `overflow-y: scroll`（避免破壞 iOS Safari 捲動體驗；統一在 `assets/css/style.scss` 管理）
